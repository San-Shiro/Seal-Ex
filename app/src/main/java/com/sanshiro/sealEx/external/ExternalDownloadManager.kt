...
import com.sanshiro.sealEx.IDownloadCallback
...
class ExternalDownloadManager(
    ...
    private fun findTemplateByName(name: String): CommandTemplate? {
        ...
        return com.sanshiro.sealEx.util.PreferenceUtil.templateListStateFlow.value
    ...
    private fun processStateChange(task: Task, state: Task.State) {
        ...
        when (downloadState) {
            is DownloadState.FetchingInfo -> {
                // No callback for fetching start â€” the client already knows from requestDownload
            }

            DownloadState.ReadyWithInfo -> {
                // Info has been fetched â€” dispatch onInfoFetched
